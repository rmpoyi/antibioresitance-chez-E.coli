...
antibio_res %>% #distribution des resistances d'atb par isolats (a modifer)
  boxplot(.)
...
top10_freq_profils_res <- top_n(freq_profils_res, 10, freq)
sum(top10_freq_profils_res[,3])/dim(atb2018_res)[1] #correspond à 70% des resistances 
top10_freq_profils_res %>%  #changer legende abscisse
  ggplot(., aes(x=Profile, y= freq)) + 
  geom_col()
...
top_mean_res_2019 %>%  #changer legende abscisse
...
top10_freq_profils_res_2019 <- top_n(freq_profils_res_2019, 10, freq)
sum(top10_freq_profils_res_2019[,3])/dim(atb2019_res)[1] #correspond à ~62% des resistances 
top10_freq_profils_res_2019 %>%  #changer legende abscisse
  ggplot(., aes(x=Profile, y= freq)) + 
  geom_col() + 
...
top_mean_res_2020 %>%  #changer legende abscisse
  ggplot(., aes(x=row.names(.), y= eSupport)) + 
  geom_col() +
...
top10_freq_profils_res_2020 <- top_n(freq_profils_res_2020, 10, freq)
sum(top10_freq_profils_res_2020[,3])/dim(atb2020_res)[1] #correspond à 62% des resistances 
top10_freq_profils_res_2020 %>%  #changer legende abscisse
  ggplot(., aes(x=Profile, y= freq)) + 
  geom_col() + 
...
top_mean_res_2021 %>%  #changer legende abscisse
  ggplot(., aes(x=row.names(.), y= eSupport)) + 
  geom_col() +
...
top10_freq_profils_res_2021 <- top_n(freq_profils_res_2021, 10, freq)
sum(top10_freq_profils_res_2021[,3])/dim(atb2021_res)[1] #correspond à 62% des resistances 
top10_freq_profils_res_2021 %>%  #changer legende abscisse
  ggplot(., aes(x=Profile, y= freq)) + 
  geom_col()  + 
  labs(x = "Profils de resistance", y = "Frequence", title = "Les 10 profils de resistances les plus frequents en 2020") +
  guides(x = guide_axis(angle = 30))

#refaire les mêmes calculs en supprimant les ATB avec le moins de 10% de souches testees 
